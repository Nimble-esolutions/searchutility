...
from .observability import (
    ...
    trace_database_operation, search_metrics, create_span
)
...
@trace_database_operation("dashboard")
def dashboard(request, folder_id=None, subfolder_id=None):
    role = getattr(request.user, 'role', 'user')

    with create_span("dashboard.view") as span:
        span.set_attribute("user_id", str(request.user.id))
        span.set_attribute("user_role", role)
        span.set_attribute("folder_id", folder_id)
        span.set_attribute("subfolder_id", subfolder_id)
        ...
        if subfolder_id:
            ...
            if request.method == "POST":
                ...
                if form.is_valid():
                    ...
                    try:
                        ...
                        # Record PDF upload metrics
                        search_metrics.record_pdf_upload(
                            file_name=pdf.title,
                            file_size=pdf.file.size if pdf.file else 0,
            ...
            span.set_attribute("pdfs_count", pdfs.count())
        ...
        if folder_id:
            ...
            subfolders = Folder.objects.filter(parent=category)\
                                       ...
                                       .order_by('name')
            span.set_attribute("subfolders_count", subfolders.count())
        ...
        categories = Folder.objects.filter(parent=None)\
                                   ...
                                   .order_by('name')
        span.set_attribute("categories_count", categories.count())
...
@trace_document_search("search_query")
def search_query(request):
    if request.method == "GET":
        with create_span("search.welcome_message") as span:
        ...
        with create_span("search.query_processing") as span:
            try:
                ...
                span.set_attribute("query_keywords_count", len(query_keywords))
                ...
                span.set_attribute("cache_key", cached_key)
                ...
                span.set_attribute("detected_language", lang)
                ...
                span.set_attribute("total_pdfs", pdfs.count())
                ...
                span.set_attribute("matched_pdfs_count", len(matched_pdfs))
                ...
                search_metrics.record_search_query(
                    query=query,
                    results_count=len(matched_pdfs),
                    duration=duration,
                    user_id=str(user_id),
                    folder_id=None
                )
